...
FEATURE_ORDER = list(load_medical_ranges().keys())


def get_reference_ranges() -> dict[str, list[float]]:
    return load_medical_ranges()


def midpoint_fill(values: dict[str, float]) -> dict[str, float]:
    completed = dict(values)
    for feature, (low, high) in get_reference_ranges().items():
        if feature not in completed:
            completed[feature] = round((low + high) / 2, 4)
    return completed


def build_model_output(patient_values: dict[str, float]) -> dict[str, Any]:
    model = load_model()
        ...
        "model_metadata": {
            ...
            "feature_importances": {
                ...
                for feature, importance in zip(
                    FEATURE_ORDER, getattr(model, "feature_importances_", np.zeros(len(FEATURE_ORDER)))
                )
            },
        },
        "inference": {
    ...
    try:
        ...
        output["explainability"] = {
            "error": f"SHAP generation failed: {exc}",
        }
