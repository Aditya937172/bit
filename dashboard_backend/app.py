...
from .chat_service import answer_case_question
from .clinical_facts import build_available_details, build_medication_guidance
from .presentation import build_dashboard_profile, build_payload_from_legacy_row, load_demo_payload
from .storage import (
    CASE_OUTPUT_DIR,
    UPLOAD_DIR,
    ensure_storage,
...
def _build_bootstrap_profiles() -> list[dict[str, Any]]:
    ...
    for row in list_legacy_patient_rows(limit=12):
        payload = build_payload_from_legacy_row(row)
        payload = _enrich_payload(f"legacy-{row['legacy_id']}", payload)
        profiles.append(build_dashboard_profile(payload, case_id=f"legacy-{row['legacy_id']}", mode="History"))

    demo_payload = _enrich_payload("demo-visualhook", load_demo_payload("demo-visualhook"))
    profiles.append(build_dashboard_profile(demo_payload, case_id="demo-visualhook", mode="Demo"))
    return profiles
...
async def chat(request: ChatRequest) -> dict[str, Any]:
    ...
    reply = answer_case_question(payload, request.message, history=persisted_history)
